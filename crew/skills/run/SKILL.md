...
3. **Flip the MR draft → ready-for-review:** `gh pr ready <MR-number>` — **only with all required checks green, or under a logged CI-unavailable outage (Step 9b.4) with the local-green note posted**; if CI has run since the Step 9b gate, re-confirm green first and **never flip over a red check** (red is a fix trigger, not an outage).
