...
- **Org-only / scope:** issue fields exist only on **org**-owned repos and need a token that can read them; on a user repo, or if the read errors with `INSUFFICIENT_SCOPES`, **warn** (`gh auth refresh -s read:project,read:org`) and fall back to oldest-first or the `priority:*` label scheme.
...
- Silently drop priority on an `INSUFFICIENT_SCOPES` error — warn and fall back to oldest-first or the `priority:*` label scheme.
