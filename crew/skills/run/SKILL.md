...
Runs only after `mr-review` clears, and **only when the ticket carries the configured `ui-label`** — otherwise skip straight to Step 11. Dispatch `crew:ui-review` to measure the built UI's whole assembled route against the design the design MCP serves, driving the stack you brought up in Step 5; a FAIL (under `ui-fidelity-mode: enforce`) is a fix trigger inside the shared cap, and a BLOCKED means the design source is missing and escalates.
...
1. Task: read the **final** `crew:reviewer`, `crew:mr-review`, and (on a UI-labelled ticket) `crew:ui-review` MR comments, extract their **non-blocking** findings (MINOR, advisory MAJOR, out-of-scope-of-this-MR), **dedup against existing open `review-followup` issues**, and file **one issue per distinct actionable finding** — labeled **`review-followup`** and **`agent-ready`** and **blocked by the source ticket** (the issue this MR `Closes`, via a GitHub blocked-by dependency on its numeric database id, so GitHub auto-unblocks it when the MR merges) — with a backlink to the MR + comment, file refs, severity. Post a short `crew:findings` summary comment on the MR listing the filed issue URLs (or "no actionable findings").
2. The filed issues are **`review-followup`- and `agent-ready`-labeled** and **blocked by the source ticket** until the MR merges, so the loop **auto-picks them up once the MR merges and unblocks them** — no human promotion needed; the blocked-by dependency is what holds them out of the loop until then.
3. **Breakpoint `findings`** → pause here.

You will not:

- Let `crew:findings` file a follow-up without the source-ticket block — the follow-up is `agent-ready`, so the blocked-by dependency is the only thing holding it out of the loop until its source MR merges.
...
- For **qa**, **reviewer**, and **ui-review**: the **running stack's base URL/port** (from Step 5) so they test against the stack you own rather than starting their own; and for **ui-review** also the **`ui-fidelity-mode`** (default `shadow`).
...
- **Findings filed:** the count of (`review-followup`- and `agent-ready`-labeled, MR-blocked) follow-up tickets `crew:findings` opened this run (with their issue #s), so the human sees what will auto-enter the loop once each source MR merges.
...
- **`ui-fidelity-mode`** — whether `crew:ui-review`'s measured MAJOR fidelity deltas gate (`enforce` → FAIL in the shared cap) or are advisory (`shadow`, default); passed into the Step 10b dispatch.
...
- After `mr-review` clears (and `crew:ui-review` has PASSed, for a UI-labelled ticket), dispatch **`crew:findings`** (Step 11) to file the advisory reviewer / mr-review / ui-review findings as **`review-followup`- and `agent-ready`-labeled, MR-blocked** follow-up tickets that auto-enter the loop once their source MR merges, before finalizing. It's non-blocking; a failure doesn't hold up the MR.
...
- _"mr-review passed, I'll finalize now — the MINOR findings are only advisory"_ — STOP. Dispatch `crew:findings` first (Step 11) to file them as **`review-followup`- and `agent-ready`-labeled, MR-blocked** follow-up tickets that auto-enter the loop once the source merges. Advisory findings shouldn't evaporate.
