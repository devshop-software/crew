...
3. **Verify the create landed (§4.11):** re-read the new issue — confirm it carries `agent-planned-label` and **not `agent-ready`** (`gh issue view <n> --json labels`). Capture its number + URL.
...
- **Populate priority** on the new bundles and any unprioritized ticket, ranked by **journey criticality** (core flow > edge case > polish), **NOT file size** — the same org **Priority Issue Field** `/crew:run` orders by (§4.5; the `priority-field` config key, fallback `priority-labels`; lower int = higher rank). **A condensed bundle inherits the MAX priority of its members** (the most critical finding sets the bundle's rank). Re-read the field to confirm it stuck.
...
- **Reconcile the board** — milestone orphan tickets (surface any that fit none → may need `/crew:plan`), populate priority by **journey criticality** (a bundle inherits the **max** priority of its members), draw `blocked_by` edges only on real ordering (the numeric DB-`id` mechanic + GET-verify), and prune stale edges.
