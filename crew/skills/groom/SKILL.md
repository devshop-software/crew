...
- **Set aside `epic-label` issues for the epic-reconciliation pass (Step 4b)** — an epic is a planning *container*, **not** a finding to fold and **not** a standalone to promote, but it is **not** off-limits like `waiting-for-human` either. Do **not** hand it to the architect; collect the open epics into a separate set and reconcile each against its sub-issues in **Step 4b** — which reconciles **both** the parent **and** its open children (the children are already planned, so the architect rightly skips them as non-inflow, but an open child carrying no promotion label is exactly the drift you heal: 4b brings them onto the promotion line). *(Lumping epics into "skip" with no reconciliation was the FT-30 bug — #276 sat open in `status-todo` with 3/16 sub-issues done; leaving that epic's open children unpromoted after advancing the parent was the FT-31 bug.)*
...
6. **Bring the epic's open children onto the promotion line (whenever the epic stays OPEN — cases 4 & 5).** A planned epic exists to be *delivered*: its **open sub-issues are the unit `/crew:run` builds**, but they only reach the run loop once a human promotes them — and only if they carry a promotion label and surface in a digest. `/crew:plan` files children `agent-planned` at creation (3.1.0+), but an **older epic, a manually-filed child, or a stripped label** leaves open children **unpromoted and invisible** — the exact limbo you exist to kill (the FT-31 bug: groom advanced #276's parent but left its 13 open children label-less, so nothing could ever pick them up). For every **open** child of this epic — **skip** closed children, any child that is itself `epic-label` (it's reconciled as its own epic), and any `waiting-for-human` or §4.13 peer-owned child (record it skipped):
   - **Backfill milestone + priority** if missing — inherit the epic's milestone; set priority by journey-criticality via the **Step-4 org issue-field recipe** (`issueFields` / `setIssueFieldValue` behind the `GraphQL-Features: issue_fields` header; FT-29). Usually already set by `/crew:plan` — verify and fill only the gaps.
   - **Label it `agent-planned`** if it carries **neither `agent-planned` nor `agent-ready`** — your own output label, the loop-closing write. `agent-planned` **auto-runs nothing and changes no priority**; it only makes the child visible to the human's in-chat promotion (Step 6), and it's reversible. A child already `agent-ready` (a human promoted it) or already `agent-planned` is left as-is. This is **not** over-reach — an open, unlabeled, fully-planned epic child is **drift, not someone's intent to preserve**; surfacing it without labeling leaves the drift in place.
   - **Resolve the children's dependency order and unblock.** Read the `blocked_by` graph among the open children (the chains `/crew:plan` drew) and **topologically sort them into promotion waves** — wave 1 = children with no open-child blocker, wave 2 = blocked only by wave 1, and so on. Where a real ordering edge is missing or stale, draw / prune it with **Step 4's numeric-DB-`id` mechanic (GET-verified)**; **non-blocking stays the default** — only draw an edge where B literally can't start until A merges. This wave order **is** the "figure out the dependencies and unblock" job: it's how the human (and `/crew:run`) see what can start now versus what waits.
   - **Stamp** each child you label / edit with one short groom-comment (`🧹 /crew:groom — epic #<epic> child · milestone <m> · priority <p> · agent-planned · wave <k>`) and **verify each write landed (§4.11)**.
   - **Carry the open children into the Step 6 digest**, nested under their epic in **wave order** — so the human promotes the batch (or just wave 1). `/crew:run` honors `blocked_by` regardless, so **promoting them all at once is safe** — blocked children simply wait their turn.

**Never promote the epic *itself* to `agent-ready`** (the run loop skips epics — the *subtasks* are the unit a human promotes), **never fold an epic into a bundle**, and **never close an epic that still has an open child.** This pass reconciles **both** the parent (items 1–5) **and** its open children's promotion readiness (item 6) — the children are **not** reconciled by Steps 2–4, because Step 1's inflow survey deliberately skips an epic's already-planned children as non-inflow, so **4b is the only place they're brought onto the promotion line.** Promoting a child to `agent-ready` is still the **human's keystroke in Step 6** — item 6 only files `agent-planned` and surfaces them in dependency order.
...
- A **numbered list** of the tickets ready for promotion — each with its `#`, title, **priority**, **milestone**, and any **`blocked_by`** chain — so the human can scan and pick. **This list INCLUDES every open epic-child you brought to `agent-planned` in Step 4b item 6**, nested under its epic in wave order — they are promotable exactly like any other `agent-planned` ticket.
...
- **Epic progress:** one line per open epic reconciled — `#<n> "<title>" — <closed>/<total> sub-issues done · <status> · priority <p>` — **followed by a nested, wave-ordered list of its open children now `agent-planned` and promotable** (`  ↳ #<c> "<title>" · <priority> · blocked_by #A (or none) · wave <k>`); promoting the whole set at once is safe (`/crew:run` honors `blocked_by`, so blocked children just wait their turn). Plus any epic **closed as complete** this pass and any surfaced as **ambiguous linkage** (no detectable sub-issues).
...
- **Epics:** open epics reconciled — closed `<N>` complete · advanced `<M>` to in-progress · `<K>` surfaced (ambiguous linkage) · **`<P>` open epic-children brought onto the promotion line** (labeled `agent-planned`, wave-ordered, carried into the digest).
...
- **Skip an epic as "out of scope"** — an epic isn't a finding, but it's **not** off-limits like `waiting-for-human`; reconcile it in Step 4b. Never **promote** an epic to `agent-ready` or **fold** it into a bundle (the run loop skips epics; the subtasks are the unit), and never **close an epic with an open child**. And don't stop at the parent — **never leave an open epic's children unpromoted**: advancing the parent is half the job; an epic whose open children carry no promotion label is still invisible to `/crew:run`. Bring them onto the promotion line in item 6 (labeling them `agent-planned` is **not** auto-promotion — `agent-ready` stays the human's Step-6 keystroke).
...
- _"I advanced the epic to in-progress, so #276 is reconciled."_ — STOP. Advancing the **parent** is only half of Step 4b. Its **open children** still carry no promotion label, so they're invisible to the digest **and** to `/crew:run`. Run **item 6**: label each unpromoted open child `agent-planned`, order them by `blocked_by` into promotion waves, and carry them into the digest. Leaving them unpromoted after advancing the parent is the **FT-31** bug.
- _"Mass-labeling these 13 planned children feels like over-reach — I'll just surface them and let the human label."_ — STOP. An open, fully-planned epic child with no promotion label is **drift, not intent.** `agent-planned` **auto-runs nothing and changes no priority** — it only restores visibility for the human's promotion, and it's reversible. Surfacing **without** labeling leaves the exact limbo groom exists to close. (You still never write `agent-ready` yourself — that's Step 6's human keystroke.)
