...
4. **Set the Priority field** on every ticket via the org `priority-field` (fallback `priority-labels`) — the same field `/crew:run` orders by (§4.5). This is the **first time priority gets populated at all**, so ordering finally means something; re-read to verify each.
...
- **Populate Priority** via the org `priority-field` (fallback `priority-labels`) — the same field `/crew:run` orders by (§4.5); default rank = **journey criticality**, not file size.
